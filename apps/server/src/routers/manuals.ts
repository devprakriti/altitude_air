...
export const manualsRouter = new Elysia({
  ...
  .get(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Retrieve company manuals for an organization with pagination and search",
      },
    }
  )
  .put(
  ...
  .post(
    ...
    async ({ params, body, user, session }) => {
      ...
      const existing = await ManualService.getStaticDocumentByKey(
        key,
        session.activeOrganizationId || "default"
      );
      if (existing) {
        throw new Error(
          "Static document already exists. Use update endpoint instead."
        );
      }
  ...
  .get(
    ...
    async ({ params, user, session }) => {
      ...
      const document = await ManualService.getStaticDocumentByKey(
        key,
        session.activeOrganizationId || "default"
      );
      if (!document) {
  ...
  .get(
    ...
    async ({ params, user, session, set }) => {
      ...
      if (
        document.organizationId !== (session.activeOrganizationId || "default")
      ) {
        set.status = 403;
  ...
  .get(
    ...
    async ({ set }) => {
      ...
      set.headers = {
        ...
        "Content-Disposition":
          "attachment; filename=manual_items_template.xlsx",
      };

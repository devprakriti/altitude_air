...
export const todoRouter = new Elysia({ 
	...
	.post("/", async ({ body }) => {
		...
		detail: {
			...
			description: "Create a new todo item with the provided text. Requires authentication.",
		}
	})
	.put("/:id/toggle", async ({ params, body }) => {
		...
		detail: {
			...
			description: "Update the completion status of a specific todo. Requires authentication.",
		}
	})
	.delete("/:id", async ({ params }) => {
		return await db.delete(todo).where(eq(todo.id, Number(params.id))).returning();
	}, {
		admin: true,
		params: t.Object({ id: t.Numeric() }),
		response: {
		...
		detail: {
			...
			description: "Delete a specific todo by its ID. Requires admin role only.",
		}
	});

...
export const aircraftMaintenanceProgram = pgTable("aircraft_maintenance_program", {
...
export const outOfPhaseInspection = pgTable("out_of_phase_inspection", {
	id: serial("id").primaryKey(),
	programId: integer("program_id").notNull().references(() => aircraftMaintenanceProgram.id),
...
// Relations
export const aircraftMaintenanceProgramRelations = relations(aircraftMaintenanceProgram, ({ many }) => ({
	outOfPhaseInspections: many(outOfPhaseInspection),
}));

export const outOfPhaseInspectionRelations = relations(outOfPhaseInspection, ({ one }) => ({

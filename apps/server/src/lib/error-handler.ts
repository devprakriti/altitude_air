...
export function createGlobalErrorHandler() {
  return new Elysia({ name: "global-error-handler" }).onError(
    ({ error, set, request }) => {
      const _errorMessage =
...
export function createValidationMiddleware() {
  return new Elysia({ name: "validation-middleware" }).onBeforeHandle(
    ({ request, set }) => {
    ...
    ({ request, set }) => {
      ...
      if (!current) {
        ...
        set.headers["Retry-After"] = Math.ceil(
          (current.resetTime + windowMs - now) / 1000
        ).toString();
        throw new RateLimitError(
          `Rate limit exceeded. Try again in ${Math.ceil((current.resetTime + windowMs - now) / 1000)} seconds.`
        );
      } else {
        current.count++;
      }

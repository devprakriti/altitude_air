...
export type TechnicalLibraryData = {
  items: Array<{
    sn?: string;
    name?: string;
    issueDate?: string;
...
export class PDFService {
   ...
   */
  static async generateTrainingOverviewPDF(
      ...
      content: [
        ...
        },
        {
          columns: [
            ...
            {
              text: `Training Plan Overview ${data.year || ""}`,
              alignment: "right",
              fontSize: 9,
            },
          ],
    ...
    return PDFService.generatePDFBuffer(docDefinition);
  }

  /**
   * Generate Technical Library PDF
   */
  static async generateTechnicalLibraryPDF(
    data: TechnicalLibraryData
  ): Promise<Buffer> {
    const body: Array<Array<{ text: string; bold?: boolean }>> = [
      [
        { text: "S/N", bold: true },
   ...
   */
  static async generateSimpleDocumentPDF(
    title: string,
    content: string
  ): Promise<Buffer> {
    const docDefinition: TDocumentDefinitions = {

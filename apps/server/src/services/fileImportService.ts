...
export class FileImportService {
	...
	static async parseCSVFile(file: File): Promise<ManualItem[]> {
		try {
			...
			return new Promise((resolve, reject) => {
				...
				stream
					.pipe(csv({
						headers: ['sn', 'name', 'issueDate', 'issueNo', 'revDate', 'revNo', 'location']
					}))
					.on('data', (row: any) => {
						items.push({
							sn: row.sn || '',
	...
	static async parseTrainingExcelFile(file: File): Promise<TrainingData[]> {
		try {
			...
			throw new Error(`Failed to parse training Excel file: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
	...
	static generateSampleCSV(): string {
		const headers = ['S/N', 'Name', 'Issue Date', 'Issue No', 'Rev Date', 'Rev No', 'Location'];
		const sampleData = [
			['1', 'Sample Manual 1', '2024-01-01', '001', '2024-01-15', 'A', 'Library A'],
			['2', 'Sample Manual 2', '2024-01-02', '002', '2024-01-16', 'B', 'Library B']
		];

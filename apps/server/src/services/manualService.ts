...
import type { TrainingOverviewData, TechnicalLibraryData } from "./pdfService";
import { FileImportService } from "./fileImportService";
import type { ManualItem, TrainingData } from "./fileImportService";
...
export interface StaticDocumentMapping {
	id?: number;
	key: string;
	documentId: number;
	title: string;
...
export class ManualService {
	...
	static async createCompanyManual(data: {
		...
		return await db.insert(companyManuals).values({
			title: data.title,
	 ...
	 */
	static async updateStaticDocument(key: string, data: {
		documentId?: number;
		title?: string;
		jsonPayload?: any;
		updatedBy: string;
		...
		return await db
			...
			.set({
				...
				description: data.jsonPayload ? JSON.stringify(data.jsonPayload) : undefined,
				uploadedBy: data.updatedBy,
				updatedAt: new Date()
			})
			.where(
	...
	static async generateStaticPDF(
		...
		switch (key) {
			case 'training-overview':
				pdfBuffer = await PDFService.generateTrainingOverviewPDF(data as TrainingOverviewData);
				break;
			case 'technical-library':
				pdfBuffer = await PDFService.generateTechnicalLibraryPDF(data as TechnicalLibraryData);
				break;
			default:
		...
		const existingDoc = await this.getStaticDocumentByKey(key, organizationId);
		
		if (existingDoc) {
			// Update existing document
			await this.updateStaticDocument(key, {
				title: `${key}.pdf`,
				jsonPayload: data,
				updatedBy: userId,
				organizationId
			...
			// Create new document
			const [newDoc] = await this.createCompanyManual({
				title: `${key}.pdf`,
	...
	static async processFileUpload(
		...
		const [document] = await this.createCompanyManual({
	 ...
	 */
	static async getFileStatsFromS3(fileKey: string): Promise<{ size: number; exists: boolean }> {
		try {
			const exists = await s3Client.exists(fileKey);
			if (!exists) {
